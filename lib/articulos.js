...
function emdashPostToArticulo(post, seccion = 'general') {
  ...
  return {
    ...
    cuerpo:           Array.isArray(data.content)
        ? data.content.map(b => `<p>${(b.children?.map(c => c.text).join('') || '').replace(/^\d+\.\s+/, '')}</p>`).join('')
                        : data.content || '',
    seccion,
    tags:             [],
...
export async function getArticulos(limit = 200) {
  try {
    ...
    // Deduplicar: extraer palabras clave (>4 letras) y agrupar por las 3 primeras
    const stopwords = new Set(['para','como','sobre','entre','desde','hasta','este','esta','estos','estas','con','los','las','del','que','por','una','uno','son','sus','más'])
    const vistos = new Set()
    const unicos = articulos.filter(Boolean).filter(a => {
      const palabras = (a.titular || '').toLowerCase().split(/\s+/)
        .map(p => p.replace(/[^a-záéíóúüñ]/g, ''))
        .filter(p => p.length > 4 && !stopwords.has(p))
        .slice(0, 3).join(' ')
      if (!palabras || vistos.has(palabras)) return false
      vistos.add(palabras)
      return true
    })
    return unicos
